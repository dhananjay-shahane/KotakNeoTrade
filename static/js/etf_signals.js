...
// Symbol Search Modal Functions
var selectedSymbol = '';

function searchSymbols() {
    var searchTerm = document.getElementById('symbolSearchInput').value.toLowerCase();
    var resultsContainer = document.getElementById('symbolSearchResults');
    ...
    var matchingSymbols = [];
    if (window.etfSignalsManager && window.etfSignalsManager.signals) {
        matchingSymbols = window.etfSignalsManager.signals.filter(function(signal) {
            var symbol = (signal.Symbol || signal.etf || signal.symbol || '').toLowerCase();
    ...
    var uniqueSymbols = [];
    var seenSymbols = {};
    matchingSymbols.forEach(function(signal) {
        var symbol = signal.Symbol || signal.etf || signal.symbol;
        if (symbol && !seenSymbols[symbol]) {
            seenSymbols[symbol] = true;
            uniqueSymbols.push(signal);
        }
    });
    ...
    var resultsHtml = '';
    uniqueSymbols.slice(0, 10).forEach(function(signal) {
        var symbol = signal.Symbol || signal.etf || signal.symbol;
        var cmp = signal.CMP || signal.cmp || 0;
        var change = signal['%CHAN'] || signal.chan || '0.00%';
        var ep = signal.EP || signal.ep || 0;
        
        resultsHtml += `
            <div class="d-flex justify-content-between align-items-center p-2 border-bottom border-secondary symbol-result" 
                 style="cursor: pointer;" 
                 onclick="selectSymbolFromResults('${symbol}')">
                <div>
                    <strong class="text-light">${symbol}</strong>
                    <small class="text-muted d-block">Entry: ₹${parseFloat(ep).toFixed(2)}</small>
                </div>
                <div class="text-end">
                    <span class="text-light">₹${parseFloat(cmp).toFixed(2)}</span>
                    <small class="d-block ${change.includes('-') ? 'text-danger' : 'text-success'}">${change}</small>
                </div>
            </div>
        `;
    });
...
function selectSymbolFromResults(symbol) {
    selectedSymbol = symbol;
    document.getElementById('symbolSearchInput').value = symbol;
    
    // Highlight selected result
    var results = document.querySelectorAll('.symbol-result');
    results.forEach(function(result) {
        result.classList.remove('bg-primary');
    });
    
    // Find and highlight the selected result
    results.forEach(function(result) {
        if (result.textContent.includes(symbol)) {
...
function selectSymbol(symbol) {
    selectedSymbol = symbol;
    document.getElementById('symbolSearchInput').value = symbol;
    searchSymbols(); // Update results to show the selected symbol
}
...
function applySymbolSearch() {
    if (selectedSymbol) {
        // Apply the symbol search to the main search input
        document.getElementById('signalSearch').value = selectedSymbol;
        ...
        // Close the modal
        var modal = document.getElementById('symbolSearchModal');
        ...
        showSwalMessage(`Search applied for symbol: ${selectedSymbol}`, 'success');
    } else {
        showSwalMessage('Please select a symbol first', 'warning');
    }
}
