function MarketWatchManager() {
    ...
    this.refreshIntervalTime = 30000; // 30 seconds for market data
...
MarketWatchManager.prototype.getDefaultColumns = function () {
    var defaultCols = [];
...
MarketWatchManager.prototype.setupColumnSettingsModal = function () {
    var self = this;
...
MarketWatchManager.prototype.setupDealButtonListeners = function () {
    var self = this;
...
MarketWatchManager.prototype.generateColumnCheckboxes = function () {
...
MarketWatchManager.prototype.updateTableHeaders = function () {
...
MarketWatchManager.prototype.getColumnTooltip = function (column) {
    var tooltips = {
...
MarketWatchManager.prototype.setupEventListeners = function () {
    // Filter event listeners can be added here
};

MarketWatchManager.prototype.loadMarketWatch = function () {
    var self = this;
...
MarketWatchManager.prototype.getGradientBackgroundColor = function (value) {
...
MarketWatchManager.prototype.renderMarketWatchTable = function () {
    var tbody = document.getElementById("dealsTableBody");
    var startIndex = (this.currentPage - 1) * this.pageSize;
    var endIndex = startIndex + this.pageSize;
    var pageDeals = this.filteredMarketData.slice(startIndex, endIndex);
    ...
    if (visibleDealsCount)
        visibleDealsCount.textContent = this.filteredMarketData.length;
    if (showingCount)
        showingCount.textContent = Math.min(
            endIndex,
            this.filteredMarketData.length,
        );
    if (totalCount) totalCount.textContent = this.filteredMarketData.length;
};

MarketWatchManager.prototype.updatePagination = function () {
    var totalPages = Math.ceil(this.filteredMarketData.length / this.pageSize);
...
MarketWatchManager.prototype.startAutoRefresh = function () {
    ...
    if (this.autoRefresh) {
        ...
        this.refreshInterval = setInterval(function () {
            ...
            if (!document.hidden && !self.isLoading) {
                self.loadMarketWatch();
            }
        }, this.refreshIntervalTime);
    }
};

MarketWatchManager.prototype.stopAutoRefresh = function () {
...
MarketWatchManager.prototype.showError = function (message) {
    var tbody = document.getElementById("dealsTableBody");
    tbody.innerHTML =
        "<tr>" +
        '<td colspan="' +
        ...
        "</h6>" +
        '<div class="mt-3">' +
        '<button class="btn btn-outline-primary btn-sm me-2" onclick="window.marketWatchManager.loadMarketWatch()">' +
...
MarketWatchManager.prototype.showLoadingSpinner = function () {
    var tbody = document.getElementById("dealsTableBody");
    tbody.innerHTML =
        "<tr>" +
        '<td colspan="' +
...
MarketWatchManager.prototype.showEmptyStateMessage = function () {
    var tbody = document.getElementById("dealsTableBody");
    tbody.innerHTML =
        "<tr>" +
        '<td colspan="' +
...
MarketWatchManager.prototype.showEmptyUserDealsMessage = function () {
    var tbody = document.getElementById("dealsTableBody");
    tbody.innerHTML =
        "<tr>" +
        '<td colspan="' +
...
function performSearch() {
    ...
    if (!searchInput || !window.marketWatchManager) return;

    var query = searchInput.value.toLowerCase().trim();

    if (query === "") {
        // Reset to show all deals
        window.marketWatchManager.filteredDeals = window.marketWatchManager.deals.slice();
    } else {
        // Filter deals based on search query - comprehensive search
        window.marketWatchManager.filteredDeals = window.marketWatchManager.deals.filter(
            function (deal) {
    ...
    window.marketWatchManager.currentPage = 1;
    window.marketWatchManager.renderMarketWatchTable();
    window.marketWatchManager.updatePagination();
    window.marketWatchManager.updateMarketWatchCountBadge();
}
...
function selectAllColumns() {
    if (window.marketWatchManager) {
        ...
        checkboxes.forEach(function (checkbox) {
            ...
            var column = window.marketWatchManager.availableColumns.find(
                function (col) {
...
function resetDefaultColumns() {
    if (window.marketWatchManager) {
        ...
        window.marketWatchManager.availableColumns.forEach(function (column) {
            column.visible =
                column.key === "symbol" ||
                column.key === "td" ||
                column.key === "ep" ||
...
function applyColumnSettings() {
    if (window.marketWatchManager) {
        ...
        window.marketWatchManager.saveColumnSettings();
        window.marketWatchManager.updateTableHeaders();
        window.marketWatchManager.renderMarketWatchTable();
        window.marketWatchManager.updatePagination();
...
function refreshMarketWatch() {
    window.marketWatchManager.loadMarketWatch();
}

function previousPage() {
    if (window.marketWatchManager.currentPage > 1) {
        window.marketWatchManager.currentPage--;
        window.marketWatchManager.renderMarketWatchTable();
        window.marketWatchManager.updatePagination();
    }
}
...
function nextPage() {
    var totalPages = Math.ceil(
        window.marketWatchManager.filteredDeals.length / window.marketWatchManager.pageSize,
    );
    if (window.marketWatchManager.currentPage < totalPages) {
        window.marketWatchManager.currentPage++;
        window.marketWatchManager.renderMarketWatchTable();
        window.marketWatchManager.updatePagination();
    }
...
function setRefreshInterval(intervalMs, displayText) {
    ...
    window.marketWatchManager.refreshIntervalTime = intervalMs;
    document.getElementById("currentInterval").textContent = displayText;

    if (window.marketWatchManager.autoRefresh) {
        window.marketWatchManager.startAutoRefresh();
    }
    ...
    fetch("/api/place-order", {
        ...
        .then(function (data) {
            ...
            if (data && data.success) {
                ...
                setTimeout(function () {
                    if (window.marketWatchManager && !window.marketWatchManager.isLoading) {
                        window.marketWatchManager.loadMarketWatch();
                    }
                }, 2000);
            } else {
...
function exportMarketWatch() {
    var data = window.marketWatchManager.filteredDeals;
...
function applyColumnSettings() {
    ...
    if (!window.marketWatchManager) {
        console.error("MarketWatchManager not initialized");
        return;
    }

    window.marketWatchManager.selectedColumns = [];

    var columns = Object.keys(window.marketWatchManager.availableColumns);
    for (var i = 0; i < columns.length; i++) {
        var column = columns[i];
        var checkbox = document.getElementById("col_" + column);
        if (checkbox && checkbox.checked) {
            window.marketWatchManager.selectedColumns.push(column);
        }
    }

    console.log("Selected columns:", window.marketWatchManager.selectedColumns);
...
function resetDefaultColumns() {
    ...
    for (var i = 0; i < columns.length; i++) {
        ...
        if (checkbox) {
            checkbox.checked =
                window.marketWatchManager.availableColumns[column].default;
        }
    }
...
function sortDealsByColumn(column) {
    if (window.marketWatchManager) {
        window.marketWatchManager.sortDirection =
            window.marketWatchManager.sortDirection === "asc" ? "desc" : "asc";
        var direction = window.marketWatchManager.sortDirection;

        window.marketWatchManager.filteredDeals.sort(function (a, b) {
            var valueA, valueB;
...
window.addEventListener("beforeunload", function () {
    if (window.marketWatchManager && window.marketWatchManager.refreshInterval) {
        clearInterval(window.marketWatchManager.refreshInterval);
        window.marketWatchManager.refreshInterval = null;
    }
});

function initializeDeals() {
    console.log("Initializing Deals Manager...");
    window.marketWatchManager = new MarketWatchManager();

    // Load initial data
    window.marketWatchManager.loadMarketWatch();

    // Check price update status (if method exists)
    if (typeof window.marketWatchManager.checkPriceUpdateStatus === "function") {
        window.marketWatchManager.checkPriceUpdateStatus();
    }
    ...
    if (savedInterval && savedDisplay && parseInt(savedInterval) === 300000) {
        window.marketWatchManager.refreshIntervalTime = parseInt(savedInterval);
        document.getElementById("currentInterval").textContent = savedDisplay;
    } else {
        // Force 5 minute interval
        window.marketWatchManager.refreshIntervalTime = 300000;
    ...
    document.addEventListener("visibilitychange", function () {
        if (document.hidden) {
            ...
            window.marketWatchManager.stopAutoRefresh();
        } else {
    ...
    window.addEventListener("storage", function (e) {
        if (e.key === "userDeals") {
            ...
            if (!window.marketWatchManager.isLoading) {
                window.marketWatchManager.loadMarketWatch();
            }
        }
    });
...
MarketWatchManager.prototype.updateMarketWatchCountBadge = function () {
    var badge = document.getElementById("dealsCountBadge");
    if (badge) {
        badge.textContent = this.filteredMarketData.length;
    }
};
...
window.createNewWatchlist = function() {
    ...
    if (!listName) {
        Swal.fire({
            icon: 'warning',
            title: 'Name Required',
            text: 'Please enter a name for your watchlist',
            background: '#1e1e1e',
            color: '#fff'
        });
        return;
    ...
    .then(data => {
        if (data.success) {
            Swal.fire({
                icon: 'success',
                title: 'Success!',
                text: `Watchlist "${listName}" created successfully`,
                background: '#1e1e1e',
                color: '#fff',
                timer: 2000,
                showConfirmButton: false
    ...
    .catch(error => {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Network Error',
            text: 'Failed to create watchlist. Please check your connection.',
            background: '#1e1e1e',
            color: '#fff'
        });
    })
    .finally(() => {
        ...
        createBtn.innerHTML = '<i class="fas fa-plus me-1"></i>Add List';
    });
}

function loadCustomWatchlists() {
    ...
    .then(data => {
        if (data.success) {
            ...
            data.watchlists.forEach((watchlist, index) => {
                createWatchlistCard(watchlist, container);
            });
        } else {
...
function createWatchlistCard(watchlist, container) {
    const cardHtml = `
        <div class="card bg-secondary border-0 shadow-lg mb-4" id="watchlist-${watchlist.name.replace(/\s+/g, '-')}">
            <div class="card-header bg-dark border-0 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 text-light">
                    ...
                    <span class="badge bg-warning text-dark ms-2" id="count-${watchlist.name.replace(/\s+/g, '-')}">${watchlist.count}</span>
                </h5>
                <div class="d-flex gap-2 align-items-center">
                    <button 
                        class="btn btn-sm btn-success" 
                        onclick="showAddSymbolModal('${watchlist.name}')"
                    >
                        <i class="fas fa-plus me-1"></i>Add Symbol to Market Watch
                    </button>
                    <button 
                        class="btn btn-sm btn-outline-light" 
                        onclick="refreshWatchlist('${watchlist.name}')"
                    >
            ...
            <div class="card-body p-0">
                <div class="table-responsive" style="overflow-y: auto">
                    <table class="table table-dark table-hover mb-0 signals-table" id="table-${watchlist.name.replace(/\s+/g, '-')}">
                        ...
                        <tbody id="tbody-${watchlist.name.replace(/\s+/g, '-')}">
            ...
            <div class="card-footer bg-dark border-0 d-flex justify-content-between align-items-center text-light">
                <div class="d-flex align-items-center gap-3">
                    <small>Showing <span id="showing-${watchlist.name.replace(/\s+/g, '-')}">0</span> of <span id="total-${watchlist.name.replace(/\s+/g, '-')}">0</span> symbols</small>
                </div>
            </div>
        </div>
    `;
    ...
    // Load market data for this watchlist
    loadWatchlistData(watchlist.name);
}

function loadWatchlistData(listName) {
    const tableId = `tbody-${listName.replace(/\s+/g, '-')}`;
    const countId = `count-${listName.replace(/\s+/g, '-')}`;
    const showingId = `showing-${listName.replace(/\s+/g, '-')}`;
    const totalId = `total-${listName.replace(/\s+/g, '-')}`;
    ...
    .then(data => {
        ...
        if (data.success) {
                ...
                tbody.innerHTML = `
                    <tr>
                        <td colspan="10" class="text-center py-4 text-muted">
                            <i class="fas fa-info-circle me-2"></i>No symbols in this watchlist
                        </td>
                    </tr>
                `;
            } else {
                data.symbols.forEach(symbol => {
                    const row = createWatchlistSymbolRow(symbol, listName);
                    tbody.innerHTML += row;
                });
            }
            ...
            if (showingElement) showingElement.textContent = data.count;
    ...
    .catch(error => {
        console.error(`Error loading watchlist ${listName}:`, error);
        const tbody = document.getElementById(tableId);
        if (tbody) {
            tbody.innerHTML = `
                <tr>
...
function createWatchlistSymbolRow(symbol, listName) {
    const changeClass = symbol.change_pct && symbol.change_pct !== '--' ? 
        (parseFloat(symbol.change_pct) >= 0 ? 'text-success' : 'text-danger') : '';
    
    return `
        <tr>
            <td class="text-center">${symbol.id}</td>
            <td class="text-center fw-bold">${symbol.symbol}</td>
            <td class="text-center">${symbol.price_7d || '--'}</td>
            <td class="text-center">${symbol.price_30d || '--'}</td>
            <td class="text-center ${changeClass}">${symbol.change_7d_pct || '--'}</td>
            <td class="text-center ${changeClass}">${symbol.change_30d_pct || '--'}</td>
            <td class="text-center fw-bold">${symbol.cmp || '--'}</td>
            <td class="text-center ${changeClass}">${symbol.change_pct || '--'}</td>
            <td class="text-center">${symbol.change_val || '--'}</td>
            <td class="text-center">
                <button 
                    class="btn btn-sm btn-outline-danger" 
                    onclick="removeSymbolFromWatchlist('${listName}', '${symbol.symbol}')"
                    title="Remove symbol"
                >
...
window.showAddSymbolModal = function(listName) {
    ...
    // Show the existing add symbol modal
    const modal = new bootstrap.Modal(document.getElementById('addSymbolModal'));
    modal.show();
    
    // Update modal title to show which list we're adding to
        ...
        if (result.isConfirmed) {
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Removed!',
                        text: `${symbol} removed from ${listName}`,
                        background: '#1e1e1e',
                        color: '#fff',
                        timer: 2000,
                        showConfirmButton: false
            ...
            .catch(error => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Network Error',
                    text: 'Failed to remove symbol. Please check your connection.',
                    background: '#1e1e1e',
                    color: '#fff'
                });
            });
        ...
        if (result.isConfirmed) {
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Deleted!',
                        text: `Watchlist "${listName}" has been deleted`,
                        background: '#1e1e1e',
                        color: '#fff',
                        timer: 2000,
                        showConfirmButton: false
            ...
            .catch(error => {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Network Error',
                    text: 'Failed to delete watchlist. Please check your connection.',
                    background: '#1e1e1e',
                    color: '#fff'
                });
            });
...
// Allow Enter key to create watchlist
document.addEventListener('DOMContentLoaded', function() {
    const listNameInput = document.getElementById('newListNameInput');
    if (listNameInput) {
...
// Override the addSelectedSymbol function to handle custom watchlists
const originalAddSelectedSymbol = window.addSelectedSymbol;
window.addSelectedSymbol = function() {
    const targetList = window.targetWatchlistName;
    
    if (targetList) {
        // Add to custom watchlist
        addSymbolToCustomWatchlist(targetList);
    } else {
        // Use original function for default watchlist
        if (originalAddSelectedSymbol) {
            originalAddSelectedSymbol();
        }
    }
};

function addSymbolToCustomWatchlist(listName) {
    ...
    .then(data => {
        if (data.success) {
            ...
            // Clear target list name
            window.targetWatchlistName = null;
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: data.error || 'Failed to add symbol',
                background: '#1e1e1e',
                color: '#fff'
            });
        }
    ...
    .catch(error => {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Network Error',
            text: 'Failed to add symbol. Please check your connection.',
            background: '#1e1e1e',
            color: '#fff'
        });
    });
