"""
Apply fix for qt field formatting in signals data.
"""
import psycopg2
        ...
        try:
            with self.connection.cursor(
                ...
                for row in results:
                    ...
                    # Format qt (quote time) properly
                    signal['qt'] = datetime.now().strftime('%H:%M:%S') if not signal.get('qt') else str(signal.get('qt'))
                    signal['qt_formatted'] = datetime.now().strftime('%I:%M %p')
