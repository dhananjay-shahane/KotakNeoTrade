...
class DynamicUserDealsService:
        ...
        try:
            ...
            index_queries = [
                ...
                sql.SQL("CREATE INDEX IF NOT EXISTS {} ON {} (status);").format(
                    sql.Identifier(f"idx_{table_name}_status"), sql.Identifier(table_name)
                ),
            ]
    ...
    def update_deal(self, username, deal_id, updates):
        ...
        try:
            ...
            logger.info(f"✅ Updated deal {deal_id} in {table_name}, rows affected: {rows_affected}")
            return rows_affected > 0
            
        except Exception as e:
            logger.error(f"❌ Failed to update deal {deal_id} in {username} table: {e}")
            if 'conn' in locals() and conn:
                conn.rollback()
                conn.close()
            return False
    ...
    def table_exists(self, username):
        ...
        try:
            ...
            logger.error(f"❌ Failed to check if table exists for {username}: {e}")
            if 'conn' in locals() and conn:
                conn.close()
            return False
