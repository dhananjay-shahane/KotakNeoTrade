...
class EmailService:
    ...
    def __init__(self):
        ...
        self.from_email = os.environ.get('MAIL_DEFAULT_SENDER',
                                         self.admin_email)
        ...
        if self.is_configured:
            ...
            logger.warning(
                "⚠️ Email credentials not found - email service disabled")
    ...
    def send_trade_signal_notification(self, signal_data: Dict[str,
                                                               Any]) -> bool:
        ...
        try:
            ...
            if not external_users:
                logger.info(
                    "No external users with email notifications enabled found")
                return True
            ...
            for user in external_users:
                ...
                if email and self.send_email(email, subject, text_content,
                                             html_content):
                    success_count += 1

            logger.info(
                f"✅ Trade signal notification sent to {success_count}/{len(external_users)} external users"
            )
    ...
    def send_deal_creation_notification(self, user_id: str,
        ...
        try:
            ...
            email_notifications = user_data.get('email_notification')
            ...
            # Create email content
            html_content = self._create_deal_html_template(
                deal_data, 'created')
            text_content = self._create_deal_text_template(
                deal_data, 'created')

            success = self.send_email(user_email, subject, text_content,
                                      html_content)
            if success:
                logger.info(
                    f"✅ Deal creation notification sent to {user_email}")
            return success
    ...
    def setup_daily_trading_updates(self,
                                    user_id: str,
                                    send_time: str = "09:00"):
        ...
        try:
            ...
            logger.info(
                f"✅ Daily trading updates scheduled for {user_id} at {send_time}"
            )
    ...
    def _send_daily_trading_update(self, user_id: str) -> bool:
        ...
        try:
            ...
            user_email = user_data.get('alternative_email') or user_data.get(
                'email')
            ...
            # Create email content
            html_content = self._create_daily_update_html_template(
                signal_changes)
            text_content = self._create_daily_update_text_template(
                signal_changes)
    ...
    def send_deal_status_notification(self, user_id: str, deal_data: Dict[str,
                                                                          Any],
                                      action: str) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"✅ Deal {action} notification sent to {user_email}")
            return success
    ...
    def _create_trade_signal_html_template(self,
    ...
    def _create_trade_signal_text_template(self,
                                           signal_data: Dict[str, Any]) -> str:
...
This is an automated trading signal notification.
    ...
    def _create_deal_html_template(self, deal_data: Dict[str, Any],
                                   action: str) -> str:
    ...
    def _create_deal_text_template(self, deal_data: Dict[str, Any],
                                   action: str) -> str:
...
This is an automated notification. Please do not reply.
    ...
    def _create_daily_update_html_template(
            self, signal_changes: List[Dict[str, Any]]) -> str:
    ...
    def _create_daily_update_text_template(
            self, signal_changes: List[Dict[str, Any]]) -> str:
